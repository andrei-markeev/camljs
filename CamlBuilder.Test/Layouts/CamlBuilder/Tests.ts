...
ExecuteOrDelayUntilScriptLoaded(() => {
    ...
    var caml = CamlBuilder.Query().Where()
        .Any(
        CamlBuilder.Expression().TextField("Email").IsEqualTo("support@google.com"),
        CamlBuilder.Expression().TextField("Email").IsEqualTo("plus@google.com"),
    ...
    caml = CamlBuilder.Query().Where()
        .Any(
        CamlBuilder.Expression().UserField("AssignedTo").IsEqualToCurrentUser(),
        CamlBuilder.Expression().UserField("AssignedTo").IsInCurrentUserGroups()
        )
        .GroupBy("Category")
        ...
        .All(
            CamlBuilder.Expression().All(
                ...
                CamlBuilder.Expression().Any(
                    CamlBuilder.Expression().UserMultiField("TargetAudience").IsEqualTo("55"),
                    CamlBuilder.Expression().UserMultiField("TargetAudience").IsEqualTo("66")
                )
            ),
            CamlBuilder.Expression().Any(
                CamlBuilder.Expression().TextField("NotificationScope").IsEqualTo("77"),
                CamlBuilder.Expression().All(
                    CamlBuilder.Expression().TextField("NotificationScope").IsEqualTo("88"),
                    CamlBuilder.Expression().TextField("ScopeWebRelativeUrl").IsEqualTo("99")
                )
            )
        )
        .ToString();
